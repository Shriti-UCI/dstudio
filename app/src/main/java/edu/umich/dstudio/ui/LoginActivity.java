...
import edu.umich.dstudio.model.User;
...
public class LoginActivity extends BaseActivity {
    ...
    protected void onResume() {
         ...
         */
        String signupEmail = mSharedPref.getPreference(Constants.KEY_SIGNUP_EMAIL);
        ...
        if (signupEmail != null) {
             ...
             */
            mSharedPref.writePreference(Constants.KEY_SIGNUP_EMAIL, null);
        }
    }
    ...
    private class MyAuthResultHandler implements Firebase.AuthResultHandler {
        ...
        public void onAuthenticated(AuthData authData) {
            mAuthProgressDialog.dismiss();
            Log.i(LOG_TAG, provider + " Successfully authenticated");
            ...
            if (authData != null) {
                 ...
                 */
                    if (authData.getProvider().equals(Constants.GOOGLE_AUTH_PROVIDER)) {
                ...
                mSharedPref.writePreference(Constants.ID_SHAREDPREF_PROVIDER, authData.getProvider());
                mSharedPref.writePreference(Constants.KEY_ENCODED_EMAIL, Utils.encodeEmail(mEmail));
                mSharedPref.writePreference(Constants.ID_SHAREDPREF_EMAIL, mEmail);
    ...
    private void setAuthenticatedUserPasswordProvider(AuthData authData) {
        final String unprocessedEmail = authData.getProviderData().get(
                Constants.GGL_PROVIDER_EMAIL_KEY).toString().toLowerCase();
        /**
        ...
        final Firebase userRef = new Firebase(Constants.FIREBASE_URL_USERS).child(mEmail);
    }
    ...
    private void setAuthenticatedUserGoogle(AuthData authData) {
        ...
        if (mGoogleApiClient.isConnected()) {
            ...
            mSharedPref.writePreference(
                    Constants.ID_SHAREDPREF_EMAIL,
                    unprocessedEmail);
        } else {
            unprocessedEmail = mSharedPref.getPreference(
                    Constants.ID_SHAREDPREF_EMAIL);
        }
        ...
        final String userName = (String) authData.getProviderData().get(
                Constants.GGL_PROVIDER_USERNAME_KEY);

        // Make a firebase user if none exists.
        ...
        userLocationRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.getValue() == null) {
                    User newUser = new User(userName, mEmail);
                    userLocationRef.setValue(newUser);
                }
            }
    ...
    private void loginWithGoogle(String token) {
        mFirebaseRef.authWithOAuthToken(
                Constants.GOOGLE_AUTH_PROVIDER, token,
                new MyAuthResultHandler(Constants.GOOGLE_AUTH_PROVIDER));
    }
    ...
    private void setupGoogleSignIn() {
        ImageButton signInButton = (ImageButton)findViewById(R.id.login_with_google);
        //Utils.invert(signInButton.getDrawable());
